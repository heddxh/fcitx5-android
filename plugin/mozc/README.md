...
2. **Build from source** (default when no prebuilt library is found). During CMake
   ...
   - apply `fcitx5-mozc/patches/protobuf.patch` to the protobuf submodule
     (libprotoc links against libupb, which fcitx5-mozc disables);
   ...
   - download the pregenerated dictionary data `mozc_data.inc` from
     ...
     offline). The dictionary is embedded into `libmozc.so`.
